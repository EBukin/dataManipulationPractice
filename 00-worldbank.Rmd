...
WITS database is a website, that uses trade and tariff data from various sources (UN Comtrade, WTO, World Bank) in order to provide users with the interface for generating own trade statistics and indicators. WITS application also provides tools to perform single and multi-market tariff cut simulations.
...
There are two main types of trade data in WITS, (1) UN Comtrade up to 6 digits trade data and (2) UN Comtrade Tariff-line data. To access UN Comtrade data we only need to log in to WITS with our user name and password. However, it we want to extract bulk Comtrade data, we may need to provide the Comtrade data access token (see more details below).

#### Advance query for trade data access

The main tool of generating user-defined trade data reports is in using the [`Home > Advanced Query > Trade Data (UN Comtrade)`](http://wits.worldbank.org/WITS/WITS/AdvanceQuery/RawTradeData/QueryDefinition.aspx?Page=RawTradeData). Here you can select any reporters, partners, commodities, periods, and classification systems in order to generate desirable table with the trade data.

Important to note. At this step our queries are limited to the maximum of 100 000 rows. In order to be able to download more than that, you either need to log in from the FAO network, or provide a UN Comtrade token to the WITS database by clicking "click here to activate" on the page [`Home > Advanced Query > Trade Data (UN Comtrade)`](http://wits.worldbank.org/WITS/WITS/AdvanceQuery/RawTradeData/QueryDefinition.aspx?Page=RawTradeData) and follow the instructions: 
...
#### User defined groups of commodities

#### User defined groups of countries.
