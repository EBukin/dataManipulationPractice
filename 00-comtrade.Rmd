...
To generate an authentication token for the UN Comtrade from the registered IP range (i.e. from the FAO headquarters), we first need to go to the UN Comtrade [autentication page](https://comtrade.un.org/data/auth/login?ReturnUrl=%2Fdata%2Fbulk) and insert our email into the `Sign in using: > IP Authentication:` field. Pressing `Authenticate my IP` will ensure we are registered in the UN Comtrade system. Please note, you need to do this only once. 
